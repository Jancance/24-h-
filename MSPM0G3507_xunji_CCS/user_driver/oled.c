...
// 按“行、列”显示字符串，例如 OLED_ShowString(1, 2, "hello", 5)
void OLED_ShowString(u8 row,u8 col,const char *chr,u8 len)
{
	u8 x;
	u8 y;
	u8 count = 0U;

	if((chr == NULL) || (len == 0U)) return;
	if((row < 1U) || (row > OLED_TEXT_MAX_ROW)) return;
	if((col < 1U) || (col > OLED_TEXT_MAX_COL)) return;

	x = (u8)((col - 1U) * (OLED_TEXT_FONT_SIZE / 2U));
	y = (u8)((row - 1U) * (OLED_TEXT_FONT_SIZE / 8U));

	while((count < len) && (*chr >= ' ') && (*chr <= '~'))
	{
		OLED_ShowChar(x,y,(u8)*chr,OLED_TEXT_FONT_SIZE);
		x += OLED_TEXT_FONT_SIZE / 2U;

		if(x > (128U - (OLED_TEXT_FONT_SIZE / 2U)))  // 自动换到下一行
		{
			x = 0U;
			y += OLED_TEXT_FONT_SIZE / 8U;
			if(y > ((OLED_TEXT_MAX_ROW - 1U) * (OLED_TEXT_FONT_SIZE / 8U))) break;
		}
